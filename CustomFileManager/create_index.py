...
def create_complete_index():
    ...
    for i, name in enumerate(levels):
        if name is not None:
            try:
                level_dict[str(i)] = get_info(name, add_published=True)
    ...
    # sort the dictionary to make the file nice to navigate in plaintext
    ids = [int(i) for i in level_dict]
    ids.sort()
    level_dict = {str(i): level_dict[str(i)] for i in ids}
...
def update_index(already_indexed_args={}, **kwargs):
    ...
    missing_keys = [key for key in level_dict if level_dict[key]['filename'] not in levels and min_id <= int(key) < max_id]
    print(missing_keys)
