...
rank_points = {'D': 1, 'C': 2, 'B': 3, 'A': 4, 'S': 5}
points_rank = {1: 'D', 2: 'C', 3: 'B', 4: 'A', 5: 'S'}
...
def get_level_candidates(level_dict, **kwargs):
    ...
    for key in level_dict:
        info = level_dict[key]
        valid = True
        valid = valid and (not min_id or min_id <= int(key))
        valid = valid and (not max_id or max_id >= int(key))
        ...
        valid = valid and (duplicate is None or duplicate == ('duplicate' in info))
        if valid:
...
def pick_level(candidates=None, **kwargs):
    ...
    choose_oldest = kwargs.get('choose_oldest', False)
...
def process_command(command, info, reason=None):
    if command == Result.LEVEL_CLEARED:
        if info['rank']:
            old_score = sum(rank_points[rank] for rank in info['rank'])
            score = sum(rank_points[rank] for rank in reason)
            if score > old_score:
                info['rank'] = reason
        else:
            info['rank'] = reason
    elif command == Result.SS:
        info['rank'] = 'SS'
        ...
        if 'hit_apples' in info:
            del info['hit_apples']
    elif command == Result.DUPLICATE:
        if reason is None: reason = input('Reason?: ')
        info['duplicate'] = reason
    return info
