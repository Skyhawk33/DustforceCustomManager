...
class Window(Frame):
    def __init__(self, master, level_dict, level_dir, splits_dir, index_file, show_stats=False):
        ...
        self.auto_submit.trace('w', self.change_auto_mode)
        next_row += 1

        Label(right, text='Result:').grid(row=next_row, column=0, sticky=W)
        values = [res.name.lower() for res in list(Result) if
                  res not in (Result.EXIT, Result.NEXT, Result.LEVEL_CLEARED)]
        self.result = StringVar(value=values[0])
        ...
        Entry(right, textvariable=self.result_comment).grid(row=next_row, column=1, columnspan=_option_columns - 1,
                                                            sticky=(N, E, S, W))
        next_row += 1
        right.grid_rowconfigure(next_row, weight=1)
    ...
    def auto_submit_callback(self, replay):
        if self.level_key is None:
            return
        info = self.level_dict.get(self.level_key, None)
        if info is None:
            return
        ...
        try:
            if replay['level'] == self.level_name:
                rank = points_rank[int(replay['score1'])] + points_rank[int(replay['score2'])]
                if mode == Result.LEVEL_CLEARED:
                    submit = True
                elif mode == Result.SS and rank == 'SS':
                    submit = True
                elif mode == Result.APPLE_SS and rank == 'SS' and int(replay.get('apples', 0)) == info['apples']:
                    submit = True
        except KeyError:
            return

        if submit:
            process_command(mode, info, rank)
            self.next_map()
            if self._show_stats:
                self.update_stats()
    ...
    def load_message(self, info, candidate_count=-1):
        ...
        for key in info:
            ...
            if key == 'level_type':
                if value == 5:
                    value = "boss"  # this level type is not implemented in LevelType
                else:
...
def main():
    ...
    _button_params.update(options.get('button_params', _button_params))
    _checkbox_params.update(options.get('checkbox_params', _checkbox_params))
    _entry_params.update(options.get('entry_params', _entry_params))
